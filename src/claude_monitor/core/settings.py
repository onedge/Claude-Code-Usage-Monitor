...
class Settings(BaseSettings):
    ...
    plan: Literal["pro", "max5", "max20", "team_standard", "team_premium", "custom"] = Field(
        default="custom",
        description="Plan type (pro, max5, max20, team_standard, team_premium, custom)",
    )
    ...
    def validate_plan(cls, v: Any) -> str:
        ...
        if isinstance(v, str):
            v_lower = v.lower()
            valid_plans = ["pro", "max5", "max20", "team_standard", "team_premium", "custom"]
